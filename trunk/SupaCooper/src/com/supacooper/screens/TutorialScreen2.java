...
public class TutorialScreen2 implements Screen {
	...
	public TutorialScreen2(SupaCooper newSupaCooper) {
		myGame = newSupaCooper;
		backgroundTexture = new Texture(Gdx.files.internal("images/backgrounds/tutorialTwo.png"));
		 ...
		 if(Gdx.input.justTouched()) {
			 if(Gdx.input.getY()<Gdx.graphics.getHeight()*0.75) {
				 if(Gdx.input.getX()>Gdx.graphics.getWidth()/2) {
					 myGame.setTutorialScreen3();
				 } else {
					 myGame.setTutorialScreen();
				 }
			 }
		 }
	}
